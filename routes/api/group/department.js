...
const fs = require("fs");
let DepartmentSchema = require('../../../models/group/department');
...
router.post('/article/upload-image',multer({ dest: 'uploads' }).any(), async (req,res) => {
    ...
    if(req.files)
    {
        ...
        var new_path = 'uploads/group/'+ department_name +'/article/'+filename+'/avatar.jpg';
        var new_folder = 'uploads/group/'+ department_name +'/article/'+filename;
...
router.post('/album/upload-album',multer({ dest: 'uploads' }).any(), async (req,res) => {
    ...
    if(req.files)
    {
        ...
        var originalname = req.files[0].originalname;
        var originalname = originalname.split('.');
        var new_path = 'uploads/group/'+ department_name +'/album/'+filename+'/avatar.jpg';
        var new_folder = 'uploads/group/'+ department_name +'/album/'+filename;
        fs.mkdir(new_folder, { recursive: true }, function(err) {
            if (err) {
              console.log(err)
            } else {
...
router.post('/blog/upload-image',multer({ dest: 'uploads' }).any(), async (req,res) => {
    ...
    if(req.files)
    {
        ...
        var originalname = req.files[0].originalname;
        var originalname = originalname.split('.');
        var new_path = 'uploads/group/'+ department_name +'/blog/'+filename+'/avatar.jpg';
        var new_folder = 'uploads/group/'+ department_name +'/blog/'+filename;
        fs.mkdir(new_folder, { recursive: true }, function(err) {
            if (err) {
              console.log(err)
            } else {
...
router.post('/article/upload-files',multer({ dest: 'uploads' }).any(), async (req,res) => {
    ...
    if(req.files)
    {
       ...
       req.files.forEach(file=>{
        ...
        var originalname = originalname.split('.');
        var new_path = 'uploads/group/article/attached_file/' + filename + '.' + originalname[originalname.length-1];
...
router.post('/blog/upload-files',multer({ dest: 'uploads' }).any(), async (req,res) => {
    ...
    if(req.files)
    {
       ...
       req.files.forEach(file=>{
        ...
        var originalname = file.originalname;
        var originalname = originalname.split('.');
        var new_path = 'uploads/group/blog/attached_file/' + filename + '.' + originalname[originalname.length-1];
        var old_path = file.path;
        var save_path =  filename + '.' + originalname[originalname.length-1];
        fs.readFile(old_path, function(err, data) {
            fs.writeFile(new_path, data, function(err) {
